const Secteurs ={
  "secteur": {
    "id": "5yjkg",
    "name": "Topo Plougastel",
    "subsecteurs": [
      {
        "secteur": {
          "id": "wsyt4",
          "img": require ('../images/secteurs/wsyt4_secteur.jpg'),
          "vignette": require ('../images/secteurs/wsyt4_vignette.jpg'),
          "access_img": require ('../images/secteurs/wsyt4_acces.jpg'),
          "name": "Le Cube",
          "description": "De Brest, prendre la RN 165 en direction de Quimper, juste après le pont de  l’Iroise, prendre la sortie Kéraliou. Au STOP prendre à droite, puis la 1ère à gauche  (petite route étroite et raide). 150 m après le virage à droite, se garer sur le petit parking.  Le chemin d’accès vers le Cube se situe juste en face du parking, celui du Serpent 500 m plus loin en prenant à gauche, puis à droite, puis de nouveau à gauche  après la deuxième maison.",
          "shortDescription": "Belle vue, une belle face",
          "subsecteurs": [
            {
              "secteur": {
                "id": "fpsvig",
                "img": require ('../images/secteurs/fpsvig_secteur.jpg'),
                "name": "Face Nord 1",
                "heigth": "25m",
                "orientation": "N",
                "routesData": "http://gsx2json.com/api?id=1jjGhJJwYKgCx2nt7clCZVE3FJPXcaeqfOqmBnObKI1A&sheet=19",
                "shortDescription": "La partie la plus raide du site",
                "description": "Comme son nom l’indique, il y fait rarement chaud mais, cette exposition lui permet  d’être sous un vent dominant qui sèche le rocher plus rapidement qu’ailleurs. De plus,  pour les amateurs de performance, toutes les voies sont des petits bijoux où il est toujours agréable de ne pas trop transpirer.",
                "routes": [
                  {
                    "number": 7,
                    "name": "Dépôt de Bilan",
                    "description": "Variante de départ sur la droite en 6b",
                    "difficulty": "6a+",
                    "equipment": "e"
                  },
                  {
                    "number": 8,
                    "name": "Super Biskoaz",
                    "description": 0,
                    "difficulty": "6a",
                    "equipment": "e"
                  },
                  {
                    "number": 9,
                    "name": "Biskoaz Kemend All",
                    "description": "Y'a un piège",
                    "difficulty": "5+",
                    "equipment": "e"
                  },
                  {
                    "number": 10,
                    "name": "La Face",
                    "description": "Un régal",
                    "difficulty": "6a",
                    "equipment": "e"
                  },
                  {
                    "number": 11,
                    "name": "Prise de Tête",
                    "description": 0,
                    "difficulty": "6b",
                    "equipment": "e"
                  },
                  {
                    "number": "11'",
                    "name": "Variante de la Prise de Tête",
                    "description": "Restez sur les Grat' jusqu'au bout",
                    "difficulty": "6c",
                    "equipment": "e"
                  },
                  {
                    "number": 0,
                    "name": "Cutting Edge",
                    "description": "Possibilité en moulinette par la voie n°11",
                    "difficulty": "7a",
                    "equipment": "c"
                  }
                ],
                "routeNumber": 7,
                "subsecteurs": [],
                "level": 2
              }
            },
            {
              "secteur": {
                "id": "1ha8wl",
                "img": require ('../images/secteurs/1ha8wl_secteur.jpg'),
                "name": "Face Nord 2",
                "heigth": "25m",
                "orientation": "N",
                "routesData": "http://gsx2json.com/api?id=1jjGhJJwYKgCx2nt7clCZVE3FJPXcaeqfOqmBnObKI1A&sheet=20",
                "shortDescription": "Un peu moins raide!",
                "description": "Cette deuxième partie de la face, beaucoup plus facile, est le complément idéal pour  les groupes où le niveau n’est pas homogène. En effet, à 10 mètres des couennes, du  facile avec une terrasse bien exposée pour prendre le soleil.",
                "routes": [
                  {
                    "number": 13,
                    "name": "Le Dièdre Blanc",
                    "description": 0,
                    "difficulty": "4+",
                    "equipment": "e"
                  },
                  {
                    "number": 14,
                    "name": "La Crotte",
                    "description": 0,
                    "difficulty": "4+",
                    "equipment": "e"
                  },
                  {
                    "number": 15,
                    "name": "Le Rige",
                    "description": 0,
                    "difficulty": 4,
                    "equipment": "e"
                  },
                  {
                    "number": 16,
                    "name": "La Reine Maka",
                    "description": "Restez bien dans la fissure",
                    "difficulty": "4+",
                    "equipment": "e"
                  }
                ],
                "routeNumber": 4,
                "subsecteurs": [],
                "level": 2
              }
            },
            {
              "secteur": {
                "id": "jghi2",
                "img": require ('../images/secteurs/jghi2_secteur.png'),
                "name": "Face Ouest",
                "heigth": "20m",
                "orientation": "O",
                "routesData": "http://gsx2json.com/api?id=1jjGhJJwYKgCx2nt7clCZVE3FJPXcaeqfOqmBnObKI1A&sheet=21",
                "shortDescription": "Le coin des débutants",
                "description": "Ce secteur facile est idéal pour l’initiation, ainsi que pour la bronzette et le cassecroûte. Bien équipé et orienté à l’ouest, c’est le paradis du samedi après midi des initiateurs.",
                "routes": [
                  {
                    "number": 17,
                    "name": "Nicolas",
                    "description": "Grand Classique d'initiation",
                    "difficulty": 3,
                    "equipment": "e"
                  },
                  {
                    "number": 18,
                    "name": "Pimprenelle",
                    "description": "Idem",
                    "difficulty": 3,
                    "equipment": "e"
                  },
                  {
                    "number": 19,
                    "name": "Roudoudou",
                    "description": "Plus fine qu'il n'y parait",
                    "difficulty": 5,
                    "equipment": "e"
                  },
                  {
                    "number": 20,
                    "name": "Fraggle Rock",
                    "description": "3 variantes de sortie",
                    "difficulty": "6a",
                    "equipment": "e"
                  },
                  {
                    "number": 21,
                    "name": "Pilou",
                    "description": 0,
                    "difficulty": 5,
                    "equipment": "e"
                  },
                  {
                    "number": 22,
                    "name": "Dévers Cool",
                    "description": "Courte voie",
                    "difficulty": 4,
                    "equipment": "e"
                  },
                  {
                    "number": 23,
                    "name": "Dévers Speed",
                    "description": "Courte voie",
                    "difficulty": 5,
                    "equipment": "e"
                  },
                  {
                    "number": 24,
                    "name": "Dévers Pépère",
                    "description": "Courte voie",
                    "difficulty": "5+",
                    "equipment": "e"
                  }
                ],
                "routeNumber": 8,
                "subsecteurs": [],
                "level": 2
              }
            },
            {
              "secteur": {
                "id": "iswgjk",
                "img": require ('../images/secteurs/iswgjk_secteur.png'),
                "name": "Face Sud",
                "heigth": "10m",
                "orientation": "S",
                "routesData": "http://gsx2json.com/api?id=1jjGhJJwYKgCx2nt7clCZVE3FJPXcaeqfOqmBnObKI1A&sheet=22",
                "shortDescription": "On dirait le Sud",
                "description": "Secteur de voies courtes entre 5 et 10 mètres. Idéal pour les débutants.",
                "routes": [
                  {
                    "number": 25,
                    "name": "P'tite Tête",
                    "description": 0,
                    "difficulty": 3,
                    "equipment": "e"
                  },
                  {
                    "number": 26,
                    "name": "P'Tit Doigt",
                    "description": 0,
                    "difficulty": 3,
                    "equipment": "e"
                  },
                  {
                    "number": 27,
                    "name": "La Nonne",
                    "description": "Eviter le petit surplomb",
                    "difficulty": 3,
                    "equipment": "e"
                  },
                  {
                    "number": 28,
                    "name": "L'évèque",
                    "description": "Passer droit dans le surplomb",
                    "difficulty": 4,
                    "equipment": "e"
                  },
                  {
                    "number": 29,
                    "name": "La Sans Nom",
                    "description": 0,
                    "difficulty": 4,
                    "equipment": "e"
                  },
                  {
                    "number": 30,
                    "name": "Le Petit Toit",
                    "description": 0,
                    "difficulty": "4+",
                    "equipment": "e"
                  },
                  {
                    "number": 31,
                    "name": "Zébulon",
                    "description": 0,
                    "difficulty": 5,
                    "equipment": "e"
                  },
                  {
                    "number": 32,
                    "name": "Fol de Moi",
                    "description": 0,
                    "difficulty": 4,
                    "equipment": "e"
                  },
                  {
                    "number": 33,
                    "name": "Glop Pas Glop",
                    "description": 0,
                    "difficulty": 4,
                    "equipment": "e"
                  }
                ],
                "routeNumber": 9,
                "subsecteurs": [],
                "level": 2
              }
            },
            {
              "secteur": {
                "id": "fufx8",
                "img": require ('../images/secteurs/fufx8_secteur.png'),
                "vignette": require ('../images/secteurs/fufx8_vignette.jpg'),
                "name": "L'Angle",
                "heigth": "25m",
                "orientation": "N/NO",
                "routesData": "http://gsx2json.com/api?id=1jjGhJJwYKgCx2nt7clCZVE3FJPXcaeqfOqmBnObKI1A&sheet=18",
                "description": "C’est le premier secteur que vous devez trouver (sauf si vous vous êtes paumés). Facile à repérer grâce au grand dièdre qui débute par un petit toit facile.  Secteur idéal en été pour les bretons et les pingouins qui craignent le soleil.",
                "routes": [
                  {
                    "number": 1,
                    "name": "La Mariton",
                    "description": "Rester bien dans la dalle pour voir",
                    "difficulty": "6c+",
                    "equipment": "e"
                  },
                  {
                    "number": 2,
                    "name": "La Variante Mariton",
                    "description": "Plus simple mais sans les points",
                    "difficulty": "6b",
                    "equipment": "c"
                  },
                  {
                    "number": 3,
                    "name": "Glups",
                    "description": "Pas de bloc au départ. Variante de sortie équipée sur la gauche (6c+)",
                    "difficulty": "6b",
                    "equipment": "e"
                  },
                  {
                    "number": 4,
                    "name": "Vas-y mon gars",
                    "description": "Sans la fissure, finesse et engagement",
                    "difficulty": "6b",
                    "equipment": "e"
                  },
                  {
                    "number": 5,
                    "name": "La Fissure",
                    "description": 0,
                    "difficulty": "3+",
                    "equipment": "c"
                  },
                  {
                    "number": 6,
                    "name": "Le Dièdre Vert",
                    "description": "En 4 si vous utilisez la fissure",
                    "difficulty": "5+",
                    "equipment": "e"
                  }
                ],
                "routeNumber": 6,
                "subsecteurs": [],
                "level": 2
              }
            },
            {
              "secteur": {
                "id": "cax1o",
                "img": require ('../images/secteurs/cax1o_secteur.jpg'),
                "name": "Pilier du Chien",
                "heigth": "6m",
                "orientation": "N",
                "routesData": "http://gsx2json.com/api?id=1jjGhJJwYKgCx2nt7clCZVE3FJPXcaeqfOqmBnObKI1A&sheet=23",
                "shortDescription": "Site fermé!",
                "description": "Danger! Ne pas grimper...\nSitué en face du secteur face Sud au niveau de « la p’tite tête » et du « p’tit doigt ».",
                "routes": [
                  {
                    "number": 34,
                    "name": "Croquette",
                    "description": "Partir du bas",
                    "difficulty": "5+",
                    "equipment": "e"
                  },
                  {
                    "number": 35,
                    "name": "La Tête du Chien",
                    "description": "Pilier Mythique",
                    "difficulty": "5+",
                    "equipment": "e"
                  },
                  {
                    "number": 36,
                    "name": "Guili Guili",
                    "description": 0,
                    "difficulty": 3,
                    "equipment": "c"
                  }
                ],
                "routeNumber": 3,
                "subsecteurs": [],
                "level": 2
              }
            }
          ],
          "level": 1
        }
      },
      {
        "secteur": {
          "id": "1crht",
          "vignette": require ('../images/secteurs/1crht_vignette.jpg'),
          "access_img": require ('../images/secteurs/1crht_acces.png'),
          "name": "Impératrice",
          "acces": "De Brest, prendre la RN 165 en direction de Quimper. Après le pont de l’Iroise prendre la 2ème sortie (Plougastel) qui mène à un 1er rond point. Faire le tour complet de celui ci en direction du «Passage» jusqu'à trouver un parking sur la droite. Ensuite, prendre le sentier qui grimpe : le rocher est immanquable.",
          "shortDescription": "Ancien nom : l'Impérator! Le rocher le plus proche de la route. De belles voies, dans tous les niveaux.",
          "description": "Ancien nom : l'Impérator! Mais historiquement l'imperator est un pilier au profil reconnaissable faisant partie du rocher de l'Impératrice...\nCe secteur est le plus proche de la route et du parking. De belles voies, dans tous les niveaux.",
          "subsecteurs": [
            {
              "secteur": {
                "id": "qlg1t",
                "img": require ('../images/secteurs/qlg1t_secteur.jpg'),
                "name": "Bloc",
                "routesData": "http://gsx2json.com/api?id=1jjGhJJwYKgCx2nt7clCZVE3FJPXcaeqfOqmBnObKI1A&sheet=13",
                "description": "Au pied de la dalle de cœur, orienté NE/SO, vous trouverez des pas de bloc en dalle  fine ou en jetés furieux.  Attention CHUTE DANGEREUSE : CRASH-PAD ET PARADE OBLIGATOIRE",
                "routes": [
                  {
                    "number": 15,
                    "name": "Destin d'un Desperado",
                    "description": 0,
                    "difficulty": 5,
                    "equipment": "b"
                  },
                  {
                    "number": 16,
                    "name": "Le Sacre",
                    "description": "Un jeté",
                    "difficulty": "6c+",
                    "equipment": "b"
                  },
                  {
                    "number": 17,
                    "name": "Crux Universalis",
                    "description": 0,
                    "difficulty": "?",
                    "equipment": "b"
                  },
                  {
                    "number": 18,
                    "name": "En attendant Godoffe",
                    "description": "Un jeté",
                    "difficulty": "?",
                    "equipment": "b"
                  },
                  {
                    "number": 19,
                    "name": "Titania",
                    "description": 0,
                    "difficulty": "4+",
                    "equipment": "b"
                  },
                  {
                    "number": 20,
                    "name": "Le Z",
                    "description": 0,
                    "difficulty": "4+",
                    "equipment": "b"
                  }
                ],
                "routeNumber": 6,
                "subsecteurs": [],
                "level": 2
              }
            },
            {
              "secteur": {
                "id": "v6u5j",
                "img": require ('../images/secteurs/v6u5j_secteur.png'),
                "name": "Dalle de Coeur",
                "heigth": "18m",
                "orientation": "NE",
                "routesData": "http://gsx2json.com/api?id=1jjGhJJwYKgCx2nt7clCZVE3FJPXcaeqfOqmBnObKI1A&sheet=12",
                "routes": [
                  {
                    "number": 11,
                    "name": "Le Vendeur de Carreaux",
                    "description": 0,
                    "difficulty": "6c",
                    "equipment": "c"
                  },
                  {
                    "number": 12,
                    "name": "Le Trèfle à Quatre Feuilles",
                    "description": 0,
                    "difficulty": "6a+",
                    "equipment": "e"
                  },
                  {
                    "number": 13,
                    "name": "La Voie du Coeur",
                    "description": "Un peu de finesse dans un monde de bourrins",
                    "difficulty": "6a",
                    "equipment": "e"
                  },
                  {
                    "number": 14,
                    "name": "Coup de Pique",
                    "description": 0,
                    "difficulty": "6b",
                    "equipment": "e"
                  }
                ],
                "routeNumber": 4,
                "subsecteurs": [],
                "level": 2
              }
            },
            {
              "secteur": {
                "id": "g3ngh",
                "img": require ('../images/secteurs/g3ngh_secteur.jpg'),
                "name": "Dalle de Verre",
                "heigth": "15m",
                "orientation": "NO",
                "routesData": "http://gsx2json.com/api?id=1jjGhJJwYKgCx2nt7clCZVE3FJPXcaeqfOqmBnObKI1A&sheet=11",
                "routes": [
                  {
                    "number": 1,
                    "name": "La route du rhum",
                    "description": 0,
                    "difficulty": "6c",
                    "equipment": "t"
                  },
                  {
                    "number": 2,
                    "name": "La Vierge",
                    "description": "Splendide",
                    "difficulty": "6c",
                    "equipment": "e"
                  },
                  {
                    "number": 3,
                    "name": "La Vierge Directe",
                    "description": "1ère par Bruno le Kangourou et pas souvent répétée",
                    "difficulty": "7c",
                    "equipment": "e"
                  },
                  {
                    "number": 4,
                    "name": "Arkhanes",
                    "description": 0,
                    "difficulty": "6b+",
                    "equipment": "b"
                  },
                  {
                    "number": 5,
                    "name": "L'Angle",
                    "description": 0,
                    "difficulty": "6a+",
                    "equipment": "b"
                  },
                  {
                    "number": 6,
                    "name": "Delirium très mince",
                    "description": "Sortie exposée (s'il vous reste des doigts jusque là)",
                    "difficulty": "7c",
                    "equipment": "b"
                  },
                  {
                    "number": 7,
                    "name": "La Dalle de Verre",
                    "description": 0,
                    "difficulty": "6c",
                    "equipment": "c"
                  },
                  {
                    "number": 8,
                    "name": "Le Vaisseau de Paul",
                    "description": 0,
                    "difficulty": "6a",
                    "equipment": "c"
                  },
                  {
                    "number": 9,
                    "name": "Tropico",
                    "description": 0,
                    "difficulty": "7b",
                    "equipment": "c"
                  },
                  {
                    "number": 10,
                    "name": "Le Vaisseau de Pierre",
                    "description": 0,
                    "difficulty": "6c",
                    "equipment": "c"
                  }
                ],
                "routeNumber": 10,
                "subsecteurs": [],
                "level": 2
              }
            },
            {
              "secteur": {
                "id": "er66b",
                "img": require ('../images/secteurs/er66b_secteur.png'),
                "name": "Le Dièdre Noir",
                "heigth": "35m",
                "orientation": "O",
                "routesData": "http://gsx2json.com/api?id=1jjGhJJwYKgCx2nt7clCZVE3FJPXcaeqfOqmBnObKI1A&sheet=14",
                "description": "Secteur sauvage, non équipé, relais sur coinceurs. Pose des coinceurs délicate sur un  rocher fragile par endroit.  Entre la Queue de Cheval et la Tuchen , sur une grande vire, un gros relais sur chaîne  d’époque à contempler pour les curieux et les nostalgiques.",
                "routes": [
                  {
                    "number": 21,
                    "name": "La Queue de Cheval",
                    "description": 0,
                    "difficulty": 5,
                    "equipment": "c"
                  },
                  {
                    "number": 22,
                    "name": "La Tuchen",
                    "description": 0,
                    "difficulty": 5,
                    "equipment": "c"
                  },
                  {
                    "number": 23,
                    "name": "Léo Narre",
                    "description": 0,
                    "difficulty": "6a+",
                    "equipment": "c"
                  },
                  {
                    "number": 24,
                    "name": "Le Dièdre noir",
                    "description": 0,
                    "difficulty": "5+",
                    "equipment": "c"
                  },
                  {
                    "number": 25,
                    "name": "L'Inconnue",
                    "description": 0,
                    "difficulty": "6a",
                    "equipment": "c"
                  }
                ],
                "routeNumber": 5,
                "subsecteurs": [],
                "level": 2
              }
            },
            {
              "secteur": {
                "id": "qw59t",
                "img": require ('../images/secteurs/qw59t_secteur.png'),
                "name": "L'X",
                "heigth": "20m",
                "orientation": "NNE",
                "routesData": "http://gsx2json.com/api?id=1jjGhJJwYKgCx2nt7clCZVE3FJPXcaeqfOqmBnObKI1A&sheet=16",
                "description": "L’accès à ce secteur se fait par le chemin sud-ouest (celui qui passe au dessus de  l’Imperator). Une superbe vue sur la rade vous y attend.",
                "routes": [
                  {
                    "number": 29,
                    "name": "L'X",
                    "description": "Excellente voie pour se perfectionner dans la pose des coinceurs",
                    "difficulty": 4,
                    "equipment": "c"
                  },
                  {
                    "number": 30,
                    "name": "Rhum Dodo",
                    "description": 0,
                    "difficulty": "6c",
                    "equipment": "c"
                  },
                  {
                    "number": 31,
                    "name": "L'Ecaille",
                    "description": "Comme son nom l'indique",
                    "difficulty": "6b",
                    "equipment": "c"
                  },
                  {
                    "number": 32,
                    "name": "Le Houx",
                    "description": 0,
                    "difficulty": 5,
                    "equipment": "c"
                  }
                ],
                "routeNumber": 4,
                "subsecteurs": [],
                "level": 2
              }
            },
            {
              "secteur": {
                "id": "3n34a",
                "img": require ('../images/secteurs/3n34a_secteur.png'),
                "name": "Le Lierre",
                "heigth": "25m",
                "orientation": "SO",
                "routesData": "http://gsx2json.com/api?id=1jjGhJJwYKgCx2nt7clCZVE3FJPXcaeqfOqmBnObKI1A&sheet=17",
                "routes": [
                  {
                    "number": 33,
                    "name": "La Cheminée",
                    "description": 0,
                    "difficulty": 4,
                    "equipment": "c"
                  },
                  {
                    "number": 34,
                    "name": "Jolly Jumper",
                    "description": 0,
                    "difficulty": "5+",
                    "equipment": "c"
                  },
                  {
                    "number": 35,
                    "name": "La Variante de la 36",
                    "description": 0,
                    "difficulty": "5+",
                    "equipment": "c"
                  },
                  {
                    "number": 36,
                    "name": "Le dièdre du Départ",
                    "description": 0,
                    "difficulty": "5+",
                    "equipment": "c"
                  },
                  {
                    "number": 37,
                    "name": "Le Mur des Lamentations",
                    "description": 0,
                    "difficulty": 5,
                    "equipment": "e"
                  },
                  {
                    "number": 38,
                    "name": "La Fissure",
                    "description": 0,
                    "difficulty": "6a+",
                    "equipment": "c"
                  },
                  {
                    "number": 39,
                    "name": "La Bienvenue",
                    "description": "Pas tant que ça",
                    "difficulty": "6b",
                    "equipment": "e"
                  },
                  {
                    "number": 40,
                    "name": "La Voie du Lierre",
                    "description": "Sans la fissure",
                    "difficulty": "6a",
                    "equipment": "e"
                  },
                  {
                    "number": 41,
                    "name": "L'Arête du Lierre",
                    "description": 0,
                    "difficulty": "6a",
                    "equipment": "e"
                  },
                  {
                    "number": 42,
                    "name": "C'L'Bloq",
                    "description": "Petit bloc déversant à droite du secteur",
                    "difficulty": "6a",
                    "equipment": "b"
                  }
                ],
                "routeNumber": 10,
                "subsecteurs": [],
                "level": 2
              }
            },
            {
              "secteur": {
                "id": "8b9zy",
                "img": require ('../images/secteurs/8b9zy_secteur.png'),
                "name": "La Magnonne",
                "heigth": "35m",
                "orientation": "NNO",
                "routesData": "http://gsx2json.com/api?id=1jjGhJJwYKgCx2nt7clCZVE3FJPXcaeqfOqmBnObKI1A&sheet=15",
                "description": "Dans ce secteur, la pose de coinceurs est possible. Pourtant il est préférable de  conseiller ces voies à des grimpeurs expérimentés en terrains d’aventure, d’autant  plus que les itinéraires sont peu évident et peuvent vite déboucher sur des passages  délicats.",
                "routes": [
                  {
                    "number": 26,
                    "name": "Léo Nain",
                    "description": 0,
                    "difficulty": "6a",
                    "equipment": "c"
                  },
                  {
                    "number": 27,
                    "name": "La Magnonne",
                    "description": "Prévoir boussole, bivouac et crampons (et un second qui puisse suivre)",
                    "difficulty": 5,
                    "equipment": "c"
                  },
                  {
                    "number": 28,
                    "name": "La Crécerelle",
                    "description": 0,
                    "difficulty": 5,
                    "equipment": "c"
                  }
                ],
                "routeNumber": 3,
                "subsecteurs": [],
                "level": 2
              }
            }
          ],
          "level": 1
        }
      },
      {
        "secteur": {
          "id": "xfwwq",
          "vignette": require ('../images/secteurs/xfwwq_vignette.jpg'),
          "access_img": require ('../images/secteurs/xfwwq_acces.png'),
          "name": "Rocher Noël",
          "acces": "De Brest, prendre la RN 165 en direction de Quimper. Après le pont de l’Iroise prendre la 2ème sortie (Plougastel) qui mène à un 1er rond point. Faire le tour complet de celui ci en direction du «Passage» jusqu'à trouver un parking sur la droite. Ensuite, prendre le sentier, et dépasser l'Impératice. C'est fléché!",
          "shortDescription": "Ancien nom : l'Impératrice! 'Grandes' voies et secteur initiation",
          "description": "Ancien nom : l'Impératrice! Les panneaux respectent la nouvelle nomenclature... 'Grandes' voies, secteur initiation, il y en a pour tous les goûts!",
          "subsecteurs": [
            {
              "secteur": {
                "id": "bh42m",
                "img": require ('../images/secteurs/bh42m_secteur.png'),
                "name": "Arbre Foudroyé ",
                "heigth": "8m",
                "orientation": "NO",
                "routesData": "http://gsx2json.com/api?id=1jjGhJJwYKgCx2nt7clCZVE3FJPXcaeqfOqmBnObKI1A&sheet=9",
                "description": "Il faut emprunter le sentier qui contourne l’Impératrice par la face sud et qui vous  emmène au sommet.  Une vue superbe sur la rade de Brest et le pont de l’Iroise vous attends. Idéal pour  prendre le thé après quelques heures d’escalade, voire pour affronter «Chipster et  Oasis» à défaut d’infusion.",
                "routes": [
                  {
                    "number": 57,
                    "name": "Batman",
                    "description": 0,
                    "difficulty": "6a",
                    "equipment": "e"
                  },
                  {
                    "number": 58,
                    "name": "Madman",
                    "description": 0,
                    "difficulty": "5+",
                    "equipment": "e"
                  },
                  {
                    "number": 59,
                    "name": "Chipster et Oasis",
                    "description": 0,
                    "difficulty": "6c",
                    "equipment": "e"
                  },
                  {
                    "number": 60,
                    "name": "Waterman",
                    "description": 0,
                    "difficulty": 4,
                    "equipment": "e"
                  }
                ],
                "routeNumber": 4,
                "subsecteurs": [],
                "level": 2
              }
            },
            {
              "secteur": {
                "id": "xoe8i",
                "img": require ('../images/secteurs/xoe8i_secteur.png'),
                "vignette": require ('../images/secteurs/xoe8i_vignette.jpg'),
                "access_img": require ('../images/secteurs/xoe8i_acces.jpg'),
                "name": "Fer à Cheval",
                "heigth": "35m",
                "orientation": "NO",
                "routesData": "http://gsx2json.com/api?id=1jjGhJJwYKgCx2nt7clCZVE3FJPXcaeqfOqmBnObKI1A&sheet=3",
                "routes": [
                  {
                    "number": 23,
                    "name": "Microbiologix",
                    "description": "Départ avec parade indispensable",
                    "difficulty": "6c",
                    "equipment": "e"
                  },
                  {
                    "number": 24,
                    "name": "La Paquita",
                    "description": "Idem",
                    "difficulty": "6c,6a+",
                    "equipment": "e"
                  },
                  {
                    "number": 25,
                    "name": "Zona",
                    "description": "Idem",
                    "difficulty": "6c+",
                    "equipment": "e"
                  },
                  {
                    "number": 26,
                    "name": "Les Luzules",
                    "description": "Idem. La classique du secteur",
                    "difficulty": "5,6a+|5",
                    "equipment": "e"
                  },
                  {
                    "number": 27,
                    "name": "Apollo XI",
                    "description": "Départ tonique",
                    "difficulty": "6a,4+",
                    "equipment": "e"
                  },
                  {
                    "number": 28,
                    "name": "Le Fer à Cheval",
                    "description": 0,
                    "difficulty": "5+/4+",
                    "equipment": "e"
                  },
                  {
                    "number": 29,
                    "name": "Variante des frangins",
                    "description": "Toute une histoire : celles des Descorps",
                    "difficulty": "6a/4+",
                    "equipment": "e"
                  },
                  {
                    "number": 30,
                    "name": "Adoigtracourcix",
                    "description": "Passage à doigts",
                    "difficulty": "6b+",
                    "equipment": "e"
                  },
                  {
                    "number": 31,
                    "name": "La Diagonale",
                    "description": 0,
                    "difficulty": "5+",
                    "equipment": "e"
                  },
                  {
                    "number": 32,
                    "name": "Demi-Portion",
                    "description": 0,
                    "difficulty": "6a/4",
                    "equipment": "e"
                  },
                  {
                    "number": 33,
                    "name": "Blanco",
                    "description": "C'est la fête des géants",
                    "difficulty": "6b+",
                    "equipment": "e"
                  },
                  {
                    "number": 34,
                    "name": "Les Bruyères",
                    "description": 0,
                    "difficulty": 5,
                    "equipment": "c"
                  }
                ],
                "routeNumber": 12,
                "subsecteurs": [],
                "level": 2
              }
            },
            {
              "secteur": {
                "id": "9c7ty",
                "img": require ('../images/secteurs/9c7ty_secteur.png'),
                "name": "Feuilles mortes",
                "heigth": "20m",
                "orientation": "SO",
                "routesData": "http://gsx2json.com/api?id=1jjGhJJwYKgCx2nt7clCZVE3FJPXcaeqfOqmBnObKI1A&sheet=5",
                "description": "Au sommet de la dalle, un relais ré-équipé vous attends.",
                "routes": [
                  {
                    "number": 40,
                    "name": "Les feuilles mortes",
                    "description": 0,
                    "difficulty": 5,
                    "equipment": "c"
                  },
                  {
                    "number": 41,
                    "name": "Pilly",
                    "description": 0,
                    "difficulty": 5,
                    "equipment": "c"
                  },
                  {
                    "number": 42,
                    "name": "P'tite Pose",
                    "description": 0,
                    "difficulty": "5+",
                    "equipment": "c"
                  }
                ],
                "routeNumber": 3,
                "subsecteurs": [],
                "level": 2
              }
            },
            {
              "secteur": {
                "id": "gk227",
                "img": require ('../images/secteurs/gk227_secteur.png'),
                "access_img": require ('../images/secteurs/gk227_acces.jpg'),
                "name": "Grand Toit",
                "heigth": "35m",
                "orientation": "NO",
                "description": "C’est ici que l’on trouve les voies les plus dures. ",
                "routesData": "http://gsx2json.com/api?id=1jjGhJJwYKgCx2nt7clCZVE3FJPXcaeqfOqmBnObKI1A&sheet=1",
                "routes": [
                  {
                    "number": 1,
                    "name": "Trinité",
                    "description": "La 2ème longueur est très aérienne",
                    "difficulty": "6a,4|5+",
                    "equipment": "e"
                  },
                  {
                    "number": 2,
                    "name": "Silence",
                    "description": "Du relais, vers l'extrémité du toit par l'angle",
                    "difficulty": "8a",
                    "equipment": "e"
                  },
                  {
                    "number": 3,
                    "name": "Les Celtiques",
                    "description": "Dur, \"conti\", et cérébral",
                    "difficulty": "8a+/8b",
                    "equipment": "e"
                  },
                  {
                    "number": 4,
                    "name": "Grand Toit",
                    "description": "Le BIG surplomb, L2 en 7a avec un \"tire clou\"",
                    "difficulty": "6c,7c",
                    "equipment": "e"
                  },
                  {
                    "number": 5,
                    "name": "Crenn Du",
                    "description": 0,
                    "difficulty": "7c+",
                    "equipment": "e"
                  },
                  {
                    "number": 6,
                    "name": "Black",
                    "description": 0,
                    "difficulty": "7c",
                    "equipment": "e"
                  },
                  {
                    "number": 7,
                    "name": "Hot Buttered Soul",
                    "description": "Pilier aérien, 2ème longueur non équipée",
                    "difficulty": "6b+,5+",
                    "equipment": "e"
                  },
                  {
                    "number": 8,
                    "name": "Zonix",
                    "description": "2ème longueur non équipée",
                    "difficulty": "6b+,5",
                    "equipment": "e"
                  },
                  {
                    "number": 9,
                    "name": "Abraracourcix",
                    "description": 0,
                    "difficulty": 5,
                    "equipment": "c"
                  },
                  {
                    "number": 10,
                    "name": "Kerozen",
                    "description": 0,
                    "difficulty": "6b",
                    "equipment": "c"
                  },
                  {
                    "number": 11,
                    "name": "Kousket Bras",
                    "description": 0,
                    "difficulty": "6c",
                    "equipment": "c"
                  },
                  {
                    "number": 12,
                    "name": "Kouign Amann",
                    "description": 0,
                    "difficulty": "6a",
                    "equipment": "c"
                  }
                ],
                "routeNumber": 12,
                "subsecteurs": [],
                "level": 2
              }
            },
            {
              "secteur": {
                "id": "yu6kn",
                "img": require ('../images/secteurs/yu6kn_secteur.jpg'),
                "vignette": require ('../images/secteurs/yu6kn_vignette.jpg'),
                "access_img": require ('../images/secteurs/yu6kn_acces.jpg'),
                "name": "Initiation 1",
                "heigth": "20m",
                "orientation": "SE",
                "routesData": "http://gsx2json.com/api?id=1jjGhJJwYKgCx2nt7clCZVE3FJPXcaeqfOqmBnObKI1A&sheet=6",
                "description": "C’est la première partie du secteur initiation de l’impératrice, avec des voies très faciles et très bien équipées. En plus il y a peu de risques de se retrouver dans une voie  dure par accident, ces dernières sont toutes déversantes à souhait",
                "routes": [
                  {
                    "number": 43,
                    "name": "La marmotte",
                    "description": 0,
                    "difficulty": 4,
                    "equipment": "e"
                  },
                  {
                    "number": 44,
                    "name": "L'Ecureuil",
                    "description": 0,
                    "difficulty": 4,
                    "equipment": "e"
                  },
                  {
                    "number": 45,
                    "name": "Babar",
                    "description": 0,
                    "difficulty": 5,
                    "equipment": "c"
                  },
                  {
                    "number": 46,
                    "name": "Colargol",
                    "description": 0,
                    "difficulty": 3,
                    "equipment": "e"
                  },
                  {
                    "number": 47,
                    "name": "Pollux",
                    "description": 0,
                    "difficulty": 3,
                    "equipment": "e"
                  },
                  {
                    "number": 48,
                    "name": "J'm'échauffe et j'reviens",
                    "description": "Attention, le plomb en sortie peut piquer le c...",
                    "difficulty": "6a",
                    "equipment": "e"
                  },
                  {
                    "number": 49,
                    "name": "Vas y voir",
                    "description": "Variante à gauche équipée en 4+(49 bis)",
                    "difficulty": "4+",
                    "equipment": "e"
                  },
                  {
                    "number": 50,
                    "name": "Facile",
                    "description": 0,
                    "difficulty": "4+",
                    "equipment": "e"
                  }
                ],
                "routeNumber": 8,
                "subsecteurs": [],
                "level": 2
              }
            },
            {
              "secteur": {
                "id": "sueeff",
                "img": require ('../images/secteurs/sueeff_secteur.jpg'),
                "vignette": require ('../images/secteurs/sueeff_vignette.jpg'),
                "access_img": require ('../images/secteurs/sueeff_acces.jpg'),
                "name": "Initiation 2",
                "heigth": "20m",
                "orientation": "SE",
                "routesData": "http://gsx2json.com/api?id=1jjGhJJwYKgCx2nt7clCZVE3FJPXcaeqfOqmBnObKI1A&sheet=7",
                "description": "Deuxième partie du secteur initiation de l’impératrice, avec, encore une fois, des  voies très faciles et très bien équipées.",
                "routes": [
                  {
                    "number": 51,
                    "name": "Bobélix",
                    "description": "Droit dans la face déversante et sans l'arête",
                    "difficulty": "7a",
                    "equipment": "e"
                  },
                  {
                    "number": 52,
                    "name": "Dégonflé",
                    "description": "Cette fois-ci, en utilisat l'arête avant le toit",
                    "difficulty": "6b",
                    "equipment": "e"
                  },
                  {
                    "number": 53,
                    "name": "Gruyères",
                    "description": 0,
                    "difficulty": 4,
                    "equipment": "e"
                  },
                  {
                    "number": 54,
                    "name": "Gouda",
                    "description": 0,
                    "difficulty": 3,
                    "equipment": "e"
                  },
                  {
                    "number": 55,
                    "name": "Mimolette",
                    "description": 0,
                    "difficulty": 3,
                    "equipment": "e"
                  },
                  {
                    "number": 56,
                    "name": "Sterno-cléido-mastodien",
                    "description": "Petit, mais costaud",
                    "difficulty": "6a",
                    "equipment": "e"
                  }
                ],
                "routeNumber": 6,
                "subsecteurs": [],
                "level": 2
              }
            },
            {
              "secteur": {
                "id": "kt0j8",
                "img": require ('../images/secteurs/kt0j8_secteur.png'),
                "access_img": require ('../images/secteurs/kt0j8_acces.jpg'),
                "name": "J'y va t'y",
                "heigth": "40m",
                "orientation": "NO",
                "routesData": "http://gsx2json.com/api?id=1jjGhJJwYKgCx2nt7clCZVE3FJPXcaeqfOqmBnObKI1A&sheet=2",
                "description": "Y’en a vraiment pour tout les goûts dans ce secteur. Alors n’hésitez pas.",
                "routes": [
                  {
                    "number": 13,
                    "name": "Obélix",
                    "description": "Départ costaud, plaisr ensuite. Le relais se trouve sur une petite vire herbeuse",
                    "difficulty": "6b+",
                    "equipment": "e"
                  },
                  {
                    "number": 14,
                    "name": "Idéfix",
                    "description": "Idem",
                    "difficulty": "6a+",
                    "equipment": "e"
                  },
                  {
                    "number": 15,
                    "name": "Black Girl",
                    "description": 0,
                    "difficulty": 4,
                    "equipment": "c"
                  },
                  {
                    "number": 16,
                    "name": "Bactériologix",
                    "description": "En continuité",
                    "difficulty": "6a",
                    "equipment": "e"
                  },
                  {
                    "number": 17,
                    "name": "J'y va t'y",
                    "description": "Engagé",
                    "difficulty": "4+,4",
                    "equipment": "e"
                  },
                  {
                    "number": 18,
                    "name": "Les abeilles",
                    "description": "Equipé dans L1",
                    "difficulty": "5+,6a",
                    "equipment": "c"
                  },
                  {
                    "number": 19,
                    "name": "Le Marais",
                    "description": "Jusqu'au sommet, la plus belle du secteur. Faites deux rappels à la descente",
                    "difficulty": "6b",
                    "equipment": "e"
                  },
                  {
                    "number": 20,
                    "name": "La Marie",
                    "description": "Sur le file de l'arête",
                    "difficulty": "6a+,4+",
                    "equipment": "e"
                  },
                  {
                    "number": 21,
                    "name": "La Directissime",
                    "description": "Un peu gazeux et à bras",
                    "difficulty": "6a,6b",
                    "equipment": "e"
                  },
                  {
                    "number": 22,
                    "name": "Variante Directissime",
                    "description": "Pour conserver des cartouches en vue du 6b (2ème longueur non équipée)",
                    "difficulty": "5+,6a",
                    "equipment": "e"
                  },
                  {
                    "number": 35,
                    "name": "La traversée des Acrobates",
                    "description": "Ne pas oublier la parade...",
                    "difficulty": "7b/c",
                    "equipment": "t"
                  }
                ],
                "routeNumber": 11,
                "subsecteurs": [],
                "level": 2
              }
            },
            {
              "secteur": {
                "id": "tyo6b",
                "img": require ('../images/secteurs/tyo6b_secteur.png'),
                "name": "Le Fennec",
                "heigth": "12m",
                "orientation": "NNE",
                "routesData": "http://gsx2json.com/api?id=1jjGhJJwYKgCx2nt7clCZVE3FJPXcaeqfOqmBnObKI1A&sheet=10",
                "description": "La légende voudrait que le Dieu blond en personne soit passé par là. La réalité quand  à elle veut que tout amoureux de la dalle passe par le Fennec.",
                "routes": [
                  {
                    "number": 61,
                    "name": "Décollage immédiat",
                    "description": "C'est fn, très fin",
                    "difficulty": "6b",
                    "equipment": 0
                  },
                  {
                    "number": 62,
                    "name": "Guipava's connection",
                    "description": 0,
                    "difficulty": "7a+",
                    "equipment": "c"
                  },
                  {
                    "number": 63,
                    "name": "Le Fennec",
                    "description": "Vous vous souviendrez des réglettes qui suivent le bi-doigt",
                    "difficulty": "7a+",
                    "equipment": "e"
                  },
                  {
                    "number": 64,
                    "name": "Le Petit Fennec",
                    "description": "Aussi bien que son ainé. Pour l'échauffement",
                    "difficulty": "6b",
                    "equipment": "e"
                  },
                  {
                    "number": 65,
                    "name": "Cochon Pendu",
                    "description": 0,
                    "difficulty": "6a",
                    "equipment": "c"
                  }
                ],
                "routeNumber": 5,
                "subsecteurs": [],
                "level": 2
              }
            },
            {
              "secteur": {
                "id": "rc4qi",
                "img": require ('../images/secteurs/rc4qi_secteur.jpg'),
                "name": "Trou de Godille",
                "heigth": "35m",
                "orientation": "NO",
                "routesData": "http://gsx2json.com/api?id=1jjGhJJwYKgCx2nt7clCZVE3FJPXcaeqfOqmBnObKI1A&sheet=4",
                "description": "Secteur où la pose des coinceurs peut être parfois difficile. A certains endroits, des  relais » sont installés.  La voie du trou de godille est intéressante et très bien équipée.",
                "routes": [
                  {
                    "number": 36,
                    "name": "La Dalle",
                    "description": 0,
                    "difficulty": "6a,4+",
                    "equipment": "c"
                  },
                  {
                    "number": 37,
                    "name": "La Pucelle",
                    "description": "On vous laisse seul juge",
                    "difficulty": "5+,4",
                    "equipment": "c"
                  },
                  {
                    "number": 38,
                    "name": "Grand Large",
                    "description": "Pour compliquer \"la pucelle\"",
                    "difficulty": "6a",
                    "equipment": "c"
                  },
                  {
                    "number": 39,
                    "name": "Trou de Godille",
                    "description": 0,
                    "difficulty": "6a",
                    "equipment": "e"
                  }
                ],
                "routeNumber": 4,
                "subsecteurs": [],
                "level": 2
              }
            }
          ],
          "level": 1
        }
      },
      {
        "secteur": {
          "id": "4nzmr",
          "img": require ('../images/secteurs/4nzmr_secteur.jpg'),
          "secteur1": require ('../images/secteurs/4nzmr_secteur1.jpg'),
          "access_img": require ('../images/secteurs/4nzmr_acces.jpg'),
          "name": "Le Serpent",
          "heigth": "20m",
          "orientation": "NE",
          "routesData": "http://gsx2json.com/api?id=1jjGhJJwYKgCx2nt7clCZVE3FJPXcaeqfOqmBnObKI1A&sheet=24",
          "description": "C’est un secteur isolé et tranquille parce qu’entouré de propriétés privées, soyez discrets et profitez tranquillement de ce petit coin de paradis. \nRécemment rééquipé, il vaut le détour!",
          "shortDescription": "Secteur intéressant, de la Dalle mais pas que!",
          "routes": [
            {
              "number": 1,
              "name": "Mords moi la queue",
              "description": "Départ avec l’angle gauche du rocher puis suivre l’arête",
              "difficulty": "5b+",
              "equipment": "e"
            },
            {
              "number": 2,
              "name": "Serpentine",
              "description": "Départ avec une écaille à gauche...faire un pas dynamique. Le reste de la\nvoie est tranquille jusqu’au relais",
              "difficulty": "5b",
              "equipment": "e"
            },
            {
              "number": 3,
              "name": "Kaa",
              "description": "Départ à côté de la voie 2. Après le second point, basculer à droite sur la grande face puis suivre le fil de l’arête",
              "difficulty": "6a+",
              "equipment": "e"
            },
            {
              "number": 4,
              "name": "Dallergie",
              "description": "Belle dalle enrichissante",
              "difficulty": "6b+",
              "equipment": "e"
            },
            {
              "number": 5,
              "name": "Pince Sans Rire",
              "description": "Echauffement des doigts",
              "difficulty": "6a+",
              "equipment": "e"
            },
            {
              "number": 6,
              "name": "Is all in your head",
              "description": "Bien suivre le tracé. Ne pas utiliser l’angle gauche.\nChauffez vos doigts et serrez bien vos chaussons !",
              "difficulty": "6b+",
              "equipment": "e"
            },
            {
              "number": 7,
              "name": "Morgane",
              "description": "C’est la fête des doigts ! Venez avec un peu de gniak et d’équilibre pour\nce festival de réglettes...et de placements",
              "difficulty": "7a/b",
              "equipment": "e"
            },
            {
              "number": 8,
              "name": "La voie de pythons",
              "description": "Restez sur la ligne. Ne pas s’aventurer vers la fissure de\ngauche. Ecailles et réglettes vous attendent",
              "difficulty": "5b",
              "equipment": "e"
            },
            {
              "number": 9,
              "name": "L’orvet",
              "description": "Voie cool. Suivez le tracé. Petit surplomb avec de bonnes prises en sortie",
              "difficulty": "5a",
              "equipment": "e"
            }
          ],
          "routeNumber": 9,
          "subsecteurs": [],
          "level": 1
        }
      }
    ],
    "level": 0
  }
};export default Secteurs;